...
describe("sessionStore", () => {
  ...
  it("round-trips captureStartedAtMs alongside a running session state", async () => {
    const storage = createInMemorySessionStorage();
    const running: PersistedSessionState = {
      sessionState: {
        status: "running",
  ...
  it("drops captureStartedAtMs when a persisted stopping state is recovered to idle", async () => {
    const storage = createInMemorySessionStorage();
    await persistState(
      {
        sessionState: {
    ...
    expect(loaded.sessionState).toEqual({ status: "idle" });
    expect(loaded.detectedSourceLanguage).toBe("en");
    expect(loaded.captureStartedAtMs).toBeUndefined();
  });
});
