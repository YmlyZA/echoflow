...
EchoFlow is a Chrome Manifest V3 extension (MVP) that captures active-tab audio, streams it to a local backend over WebSocket, and renders bilingual subtitles in the page. The backend ships with deterministic **fake** speech/translation providers; real domestic ASR adapters are not implemented yet (selecting one fails fast), and a Volcengine translation adapter exists. See `README.md` for end-user setup and Chrome loading steps.
...
`server.ts` registers the WS route and, per connection, constructs a `RealtimeSession` (`realtime/session.ts`) with speech + translation providers from `providerFactory.ts`. The session distinguishes JSON control frames from binary audio frames by inspecting `isBinary` + a `looksLikeJson` check; control frames are validated against `isClientMessage` (malformed → `invalid_client_message` error event). For each audio frame it runs `speechProvider.recognize` → emits a one-time `language` event → `translate` for partial → emits `partial` → `translate` for final → emits `final`. Providers are split (`createSpeechProvider` / `createTranslationProvider`) because streaming ASR and text translation use different APIs/credentials; non-`fake` ASR throws, and `volcengine` translation throws without `VOLCENGINE_API_KEY`. Config precedence lives in `config.ts` (explicit input → `ECHOFLOW_*` env → `PORT` fallback → defaults).
...
- `entrypoints/offscreen/main.ts` — offscreen document (`reasons: ["USER_MEDIA"]`). The only place that can run `getUserMedia`/`MediaRecorder` in MV3. Owns the `RealtimeClient` (WS to backend) and `OffscreenAudioPipeline`. Translates backend `ServerEvent`s into `SERVER_EVENT` runtime messages.
- `entrypoints/content.tsx` — **runtime-injected** content script (`registration: "runtime"`, not declared in the manifest; injected on demand via `chrome.scripting.executeScript`). Mounts a React overlay into a **shadow root** for style isolation. Receives `SERVER_EVENT` runtime messages, re-dispatches them as a `window` `CustomEvent` (`echoflow:server-event`), which the React tree reduces into subtitle state.
